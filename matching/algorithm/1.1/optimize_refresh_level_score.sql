/*
  get_levels:
  - Filter match_assessmet_view with company_id and supporter_id 
*/
drop function if exists matching.get_levels(alg_weight_id integer, alg_score integer, unanswered_factor float, high_unanswered_factor float, wrong_factor float, high_wrong_factor float);

create or replace function matching.get_levels(alg_weight_id integer, alg_score integer, unanswered_factor float, high_unanswered_factor float, wrong_factor float, high_wrong_factor float, _company_id integer DEFAULT '-1'::integer, _supporter_id integer DEFAULT '-1'::integer)
...
begin
    return query
        ...
        from (
                 ...
                 from matching.match_assessment_view am 
...
begin
    ...
    from matching.get_levels(alg_weight_id, alg_score, unanswered_factor, high_unanswered_factor, wrong_factor, high_wrong_factor, _company_id, _supporter_id) as vw;
end;
$$;