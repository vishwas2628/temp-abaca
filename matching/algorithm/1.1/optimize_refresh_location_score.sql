/*
  get_locations:
  - Filter match_location_view with company_id and supporter_id 
*/
drop function if exists matching.get_locations(alg_weight_id integer, alg_score integer, unanswered_factor float, high_unanswered_factor float, wrong_factor float, high_wrong_factor float);

create or replace function matching.get_locations(alg_weight_id integer, alg_score integer, unanswered_factor float, high_unanswered_factor float, wrong_factor float, high_wrong_factor float, _company_id integer DEFAULT '-1'::integer, _supporter_id integer DEFAULT '-1'::integer)
...
begin
    return query
        ...
        from (
                 ...
                 from matching.match_location_view am
...
/*
  refresh_level_score:
  - Call get_levels with company_id and supporter_id 
*/
...
begin
    ...
    from matching.get_locations(alg_weight_id, alg_score, unanswered_factor, high_unanswered_factor, wrong_factor, high_wrong_factor, _company_id, _supporter_id) as vw;
end;
$$;